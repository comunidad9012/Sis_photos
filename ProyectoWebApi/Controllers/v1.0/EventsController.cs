...
namespace Photography.ProyectoWebApi.Controllers.v1._0
{
    ...
    public class EventsController : BaseApiController
    {
        ...
        [HttpPut("{id:long}")]
        public async Task<IActionResult> Put(string id, UpdateEventsCommand command)
        {
            if (id != command.IdType)
                return BadRequest("Error en el Id suministrado no corresponde al registro a actualizar");
            return Ok(await Mediator.Send(command));
        }

        [HttpDelete("{id:long}")]
        public async Task<IActionResult> Delete(long IdType)
        {
            return Ok(await Mediator.Send(new DeleteEventsCommand { IdType = IdType }));
        }
    }
}